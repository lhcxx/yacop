...
import org.apache.hadoop.yarn.applications.yacop.common.DateUtil;
import org.apache.hadoop.yarn.applications.yacop.common.ImageUtil;
...
import org.apache.hadoop.yarn.applications.yacop.worker.NWorkerImpl;
...
public class NTaskImpl implements Task, EventHandler<TaskEvent>{
  ...
  public NTaskImpl(JobId jobId, int id, EventHandler eventHandler, int pri, NRegistryOperator nRegistryOperator, YacopConfig yacopConfig) {
  ...
  private static class ScheduleTransition implements
    ...
    public void transition(NTaskImpl nTask, TaskEvent taskEvent) {
      ...
      Resource capability = Resource.newInstance((int)nTask.getYacopConfig().getMem(),(int)nTask.getYacopConfig().getCpus());
  ...
  private static class SetupTransition implements
    ...
    public TaskState transition(NTaskImpl nTask, TaskEvent taskEvent) {
      ...
      if (nTask.getYacopConfig().isEngineLocalImage()) {
        ...
        String resourceName = ImageUtil.getFSFileName(nTask.getYacopConfig().getEngineImage());
        String resourcePath = ImageUtil.getFSFilePathSuffix(nTask.getYacopConfig().getName(), nTask.getAppId(), resourceName);
        String workerCmd = "docker load -i ./" + resourceName;
        NWorkerImpl worker = new NWorkerImpl(taskEvent.getTaskID(), 0,
            nTask.eventHandler, hostname, workerCmd, resourceName, resourcePath, nTask.getYacopConfig());
    ...
    private void setContainerScheduledRecord(NTaskImpl nTask) {
      ...
      nTask.nRegistryOperator.setContainerRecord(containerIdStr, YacopConstant.HOST, allocatedContainer.getNodeId().getHost());
//      nTask.nRegistryOperator.setContainerRecord(containerIdStr, YacopConstant.PORT, String.valueOf(allocatedContainer.getNodeId().getPort()));
      nTask.nRegistryOperator.setContainerRecord(containerIdStr, YacopConstant.CREATED, DateUtil.getCurrentTime());
      nTask.nRegistryOperator.setContainerRecord(containerIdStr, YacopConstant.STATUS, TaskState.SCHEDULED.toString());
  ...
  private static class LaunchTransition implements
    ...
    public void transition(NTaskImpl nTask, TaskEvent taskEvent) {
      ...
      containerLauncherEvent.setYacopConfig(nTask.getYacopConfig());
      nTask.eventHandler.handle(containerLauncherEvent);
    }

    private void setContainerRunningRecord(NTaskImpl nTask) {
      nTask.nRegistryOperator.setContainerRecord(nTask.container.getId().toString(), YacopConstant.COMMAND, nTask.getYacopConfig().getCmd());
      nTask.nRegistryOperator.setContainerRecord(nTask.container.getId().toString(), YacopConstant.IMAGE, nTask.getYacopConfig().getEngineImage());
      nTask.nRegistryOperator.setContainerRecord(nTask.container.getId().toString(), YacopConstant.STATUS, TaskState.RUNNING.toString());
  ...
  private static class CompleteTransition implements
    ...
    private void setContainerSucceedRecord(NTaskImpl nTask) {
      ...
      nTask.nRegistryOperator.setContainerRecord(startedContainerId.toString(), YacopConstant.STATUS, TaskState.SUCCEED.toString());
