...
public class TestUtils {
    ...
    public static NarwhalConfig mockNarwhalConfig(String name, String cmd, String image, double cpus, double mem, int instances, boolean localImage, List<VolumeConfig> volumeConfigList) {
        NarwhalConfig narwhalConfig = mock(NarwhalConfig.class, withSettings().serializable());

        when(narwhalConfig.getName()).thenReturn(name);
        when(narwhalConfig.getName()).thenReturn(cmd);
        when(narwhalConfig.getName()).thenReturn(image);
