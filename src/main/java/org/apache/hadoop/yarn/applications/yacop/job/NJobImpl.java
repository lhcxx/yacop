...
public class NJobImpl implements Job, EventHandler<JobEvent> {
  ...
  private static class InitTransition implements
    ...
    public JobState transition(NJobImpl nJob, JobEvent jobEvent) {
      ...
      setAppRecord(nJob);
      createTasks(nJob, jobEvent.getYacopConfig());
      return JobState.INITED;
    }
    
    private void setAppRecord(NJobImpl nJob){
      nJob.nRegistryOperator.setAppRecord(YacopConstant.CREATED, DateUtil.getCurrentTime());
      nJob.nRegistryOperator.updateApp();    	
    }

    private void createTasks(NJobImpl nJob, YacopConfig yacopConfig) {
      int taskNum = yacopConfig.getInstances();
      for (int i = 0; i < taskNum; i++) {
        Task task = new NTaskImpl(nJob.getID(), i, nJob.eventHandler, 0, nJob.nRegistryOperator, yacopConfig);
        nJob.addTask(task);
      }
